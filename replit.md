...
Template Ads is now a **TypeScript React application** that provides a visual canvas editor for creating professional advertisements. Originally built with Flask, the project has been completely converted to modern React with TypeScript for better type safety and maintainability. The application leverages Fabric.js for canvas manipulation and offers features like image uploads, text editing, template selection with multiple orientations. The original Flask code is preserved as backup in the backup-flask/ directory.
...
- June 24, 2025: Fixed reset button to properly reload default image - added loadDefaultImage call after template reset to ensure the placeholder image is restored correctly
- June 24, 2025: Fixed CTA floating toolbar functionality - corrected element ID mismatches between HTML and JavaScript, added console logging for debugging, updated reset function to properly reset title, subtitle, and CTA text values on canvas
- June 24, 2025: Fixed CTA toolbar font dropdown pre-filling - toolbar now properly displays current font family when CTA button is selected, handles font string cleaning and adds missing fonts to dropdown if needed
...
- June 25, 2025: Updated minimum scale positioning to center-left instead of top-left for better visual alignment, removed transition animations when switching aspect ratios for instant feedback
