...
def generate_ai_image():
    ...
    try:
        ...
        api_key = os.environ.get('OPENAI_API_KEY')
        if not api_key:
            return jsonify({'error': 'OpenAI API key not configured. Please check your API key.'}), 503
        
        # Validate API key format
        if not api_key.startswith('sk-'):
            return jsonify({'error': 'Invalid OpenAI API key format. Key should start with "sk-"'}), 400
        
        # Initialize OpenAI client
        client = OpenAI(api_key=api_key)
        
        # Use DALL-E 2 with supported sizes
        dalle2_sizes = ["256x256", "512x512", "1024x1024"]
        image_size = "512x512"  # Default
        
        if orientation == 'horizontal' or orientation == 'vertical':
            image_size = "1024x1024"  # Use largest available for non-square
        ...
        if len(response.data) == 0:
            return jsonify({'error': 'No image generated by OpenAI'}), 500
        ...
        error_msg = str(e)
        current_app.logger.error(f"OpenAI image generation error: {error_msg}")
        
        # Provide more specific error messages
        if "insufficient_quota" in error_msg:
            return jsonify({'error': 'OpenAI API quota exceeded. Please check your billing settings.'}), 402
        elif "invalid_api_key" in error_msg:
            return jsonify({'error': 'Invalid OpenAI API key. Please verify your key is correct.'}), 401
